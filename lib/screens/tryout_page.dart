...
class TryOutScreen extends StatefulWidget {
  ...
  @override
  State<TryOutScreen> createState() => TryOutScreenState();
}

class TryOutScreenState extends State<TryOutScreen> {
  ...
  getTimeSlots(pickdate) async {
    ...
    Map req = {
      ...
      "date": formattedpickedDate.toString(),
      "branch_id": 1
    };
    await getTimeSlotsForBooking(req).then((value) {
      ...
      if (value['ret_data'] == "success") {
        for (var bslots in value['time_slots']) {
          var timeHours = bslots['tm_start_time'];
          ...
          if (splittedTimeHours < currentHour) {
            ...
            if (splittedTimeMinutes < currentMinute) {
              return -1;
            } else if (splittedTimeMinutes > currentMinute) {
              return 1;
            } else {
              return 0;
            }
        ...
        if (formattedpickedDate == formattedDate) {
          for (var bslots in value['time_slots']) {
            var timeHours = bslots['tm_start_time'];
            ...
            if (count == value['driver_count']) {
              DateTime time1 = DateTime(timeHours); // First time
              DateTime time2 = DateTime(currentMinute); // Second time
              int comparison = time1.compareTo(time2);
              String comparisonResult;
              if (comparison == 0) {
                comparisonResult = 'The times are the same';
              } else if (comparison < 0) {
                comparisonResult = 'Time 1 is earlier than time 2';
              } else {
                comparisonResult = 'Time 1 is later than time 2';
              }
            }
          }
          // var count = value['assigned_emp']
